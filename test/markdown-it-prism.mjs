...
const tokensRef = new _g.WeakMap(); // TODO: performance tweaks for emphasis **_* pattern which has only 1/10 performance after adding line offset
...
Ruler.prototype.__compile__ = function () {
  const self = this;
  const chains = [""]; // collect unique names
...
Ruler.prototype.disable = function (list, ignoreInvalid) {
  ...
  const result = []; // Search by name and disable
...
Core.prototype.State = StateCore; // GFM table, https://github.github.com/gfm/#tables-extension-
...
function table(state, startLine, endLine, silent) {
  let ch, lineText, pos, i, l, nextLine, columns, columnCount, token, aligns, t, tableLines, tbodyLines, oldParentType, terminate, terminatorRules, firstCh, secondCh, positions, len, columnVIndex; // should have at least two lines
...
const HTML_OPEN_CLOSE_TAG_RE = new RegExp("^(?:" + open_tag + "|" + close_tag + ")"); // HTML block
...
const _rules$1 = [// First 2 params - rule name & source. Secondary array - list of rules,
...
ParserBlock.prototype.State = StateBlock; // Handle implicit links found by rules_core/linkify that were not yet
...
function markdownItPrism(markdownit, useroptions) {
  ...
  options.init(Prism); // register ourselves as highlighter
...
const ASSERT = {
  equalIgnoreSpaces: (a, b) => {
    ...
    strict.strictEqual(aa, bb);
  }
};
describe('markdown-it-prism', () => {
  it('highlights fenced code blocks with a language specification using Prism', () => {
    ASSERT.equalIgnoreSpaces(MarkdownIt().use(markdownItPrism).render(read('input/fenced-with-language.md')), read('expected/fenced-with-language.html'));
  });
  it('throws for unknown plugins', () => {
    strict.throws(() => MarkdownIt().use(markdownItPrism, {
      plugins: ['foo']
    }), /plugin/);
  });
  ...
  it('offers an init function for further initialisation', () => {
    ...
    MarkdownIt().use(markdownItPrism, {
      init: prism => {
        strict.ok(prism.hasOwnProperty('plugins'));
        called = true;
      }
    });
    strict.strictEqual(called, true);
  });
  it('does not add classes to fenced code blocks without language specification', () => {
    ASSERT.equalIgnoreSpaces(MarkdownIt().use(markdownItPrism).render(read('input/fenced-without-language.md')), read('expected/fenced-without-language.html'));
  });
  it('falls back to defaultLanguageForUnspecified if no language is specified', () => {
    ASSERT.equalIgnoreSpaces(MarkdownIt().use(markdownItPrism, {
      defaultLanguageForUnspecified: 'java'
    }).render(read('input/fenced-without-language.md')), read('expected/fenced-with-language.html'));
  });
  ...
  it('does not invoke noKnownLanguageCallback if no language and no defaultLanguage is specified', () => {
    ...
    }).render(read('input/fenced-without-language.md')), read('expected/fenced-without-language.html'));
  });
  it('invokes noKnownLanguageCallback if language is specified but unknown', () => {
    ...
    }).render(read('input/fenced-with-unknown-language.md')), /There is no Prism language/);
  });
  it('does not add classes to indented code blocks', () => {
    ASSERT.equalIgnoreSpaces(MarkdownIt().use(markdownItPrism).render(read('input/indented.md')), read('expected/indented.html'));
  });
  it('adds classes even if the language is unknown', () => {
    ASSERT.equalIgnoreSpaces(MarkdownIt().use(markdownItPrism).render(read('input/fenced-with-unknown-language.md')), read('expected/fenced-with-unknown-language.html'));
  });
  it('escapes HTML in the language name', async () => {
    ASSERT.equalIgnoreSpaces(MarkdownIt().use(markdownItPrism).render(read('input/fenced-with-html-in-language.md')), read('expected/fenced-with-html-in-language.html'));
  });
  it('falls back to defaultLanguageForUnknown if the specified language is unknown', () => {
    ASSERT.equalIgnoreSpaces(MarkdownIt().use(markdownItPrism, {
      defaultLanguageForUnknown: 'java'
    }).render(read('input/fenced-with-unknown-language.md')), read('expected/fenced-with-language.html'));
  });
  ...
  it('respects markdown-it’s langPrefix setting', () => {
    ASSERT.equalIgnoreSpaces(MarkdownIt({
      langPrefix: 'test-'
    }).use(markdownItPrism).render(read('input/fenced-with-language.md')), read('expected/fenced-with-language-prefix.html'));
  });
  it('is able to resolve C++ correctly', () => {
    ASSERT.equalIgnoreSpaces(MarkdownIt().use(markdownItPrism).render(read('input/cpp.md')), read('expected/cpp.html'));
  ...
  xit('allows to use Prism plugins', () => {
    ...
    }).render(read('input/fenced-with-language.md')), read('expected/fenced-with-language-plugins.html'));
  });
});
//# sourceMappingURL=markdown-it-prism.mjs.map
